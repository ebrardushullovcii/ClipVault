---
name: ClipVault Core Engine
overview: Build a Windows game clipping core engine in C# with rolling buffer capture, NVENC encoding, and game detection - controlled via JSON configuration and global hotkeys.
todos:
    - id: setup-project
      content: Create solution structure with ClipVault.Core and ClipVault.Service projects, add NuGet dependencies
      status: pending
    - id: config-system
      content: Implement ConfigManager with settings.json and games.json parsing
      status: pending
    - id: screen-capture
      content: Implement WindowsGraphicsCapture using Windows.Graphics.Capture API with D3D11 texture output
      status: pending
    - id: audio-capture
      content: Implement SystemAudioCapture (WASAPI loopback) and MicrophoneCapture using NAudio
      status: pending
    - id: ring-buffers
      content: Implement VideoFrameBuffer and AudioSampleBuffer with configurable duration
      status: pending
    - id: ffmpeg-encoder
      content: Implement FFmpegEncoder that pipes buffered data to FFmpeg with NVENC, outputs multi-track MP4
      status: pending
    - id: game-detector
      content: Implement GameDetector with process polling, games.json matching (150+ games pre-populated), and custom game support
      status: pending
    - id: hotkey-manager
      content: Implement global hotkey registration using Win32 RegisterHotKey
      status: pending
    - id: service-orchestrator
      content: Build ClipVaultService that ties all components together with lifecycle management
      status: pending
    - id: tray-icon
      content: Add minimal system tray icon with status indicator and exit option
      status: pending
    - id: metadata-thumbnail
      content: Implement metadata.json generation and thumbnail extraction via FFmpeg
      status: pending
isProject: false
---
...
We'll ship with a comprehensive `games.json` containing 100+ popular games. The format includes Twitch/IGDB IDs for potential future streaming integration.
...
## Pre-populated Games Database (games.json)

Ships with 100+ games. Users can add custom games via the config.

Users can add entries to `customGames` array for games not in the default list.
