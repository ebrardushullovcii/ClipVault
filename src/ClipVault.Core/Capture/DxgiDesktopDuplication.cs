...
public sealed class GdiScreenCapture : IScreenCapture, IDisposable
{
    ...
    public Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            _frameCount = 0;
            _isCapturing = true;
            Logger.Debug("GdiScreenCapture: Started");
    ...
    {
        // Force 720p to reduce memory usage (8MB -> 3.7MB per frame)
        _width = 1280;
        _height = 720;

        Logger.Debug($"Capture size: {_width}x{_height} (forced 720p for memory efficiency)");
        ...
        Logger.Debug("GDI capture initialized");
    }
        ...
        while (_isCapturing && !ct.IsCancellationRequested)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        Logger.Debug("CaptureFrame failed repeatedly, reinitializing...");
                        ReinitializeCapture();
                        retryCount = 0;
                    }
            ...
            {
                Logger.Error("Error in capture loop", ex);
                await Task.Delay(16, ct);
            }
        }
    }
    ...
    private bool CaptureFrame()
    {
        ...
        {
            Logger.Debug($"CaptureFrame error: {ex.Message}");
            return false;
        }
    }
    ...
    public Task StopAsync()
    {
        ...
        lock (_lock)
        {
            ...
            _isCapturing = false;
            Logger.Debug($"GdiScreenCapture: Stopped (captured {_frameCount} frames)");
        }
